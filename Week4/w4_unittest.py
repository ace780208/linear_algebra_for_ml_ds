...
def test_center_data(target_center_data):
    ...
    test_cases = [{'name': 'default_check',
                    ...
                    {'name': 'size_hardcoding_check',
                    'input': np.load('./support_files/hardcode_check.npy'),
                    'expected': np.load('./support_files/hardcode_check_centered.npy')
                    },
                    ]
    for test_case in test_cases:
        try:
            ...
            print(
                f"There was an error evaluating the function. \nError: {e}. Doublecheck if you are not hardcoding some data (like the shape). The function must work for any matrix and not for a matrix of specific shape.")
            return
...
def test_cov_matrix(target_cov_mat):
    ...
    test_cases = [{'name': 'default_check',
                    ...
                    {"name": 'check_1','input': np.array([[-0.33333333, -1.66666667,  0.        ],
                                            [-1.33333333,  0.33333333,  1.        ],
                                            [ 1.66666667,  1.33333333, -1.        ]]),
                         'expected': np.array([[ 2.33333333,  1.16666667, -1.5       ],
                                               [ 1.16666667,  2.33333333, -0.5       ],
                                               [-1.5       , -0.5       ,  1.        ]])
                        },
                        {'name':'check_2','input': np.array([[-1., -1., -1.],
                                            [ 0.,  0.,  0.],
                                            [ 1.,  1.,  1.]]),
